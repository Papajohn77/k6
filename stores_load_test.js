...
export default function () {
  const { longitude, latitude, max_distance } = data[scenario.iterationInTest];

  let res;
  if (scenario.iterationInTest <= warmupIterations) {
    res = http.get(
      `http://${__ENV.DOMAIN_NAME}/stores?longitude=${longitude}&latitude=${latitude}&maxDistance=${max_distance}`,
      {
        tags: { name: "Warmup" },
      }
    );
